...
const MIST_MODES = ["small", "large", "intermittent"];
...
function parseColorData(raw) {
  ...
  if (typeof raw === "string") {
    const text = raw.trim();
    // 12-char or 14-char HSV hex (HHHH SSSS VVVV [XX])
    if (/^[0-9a-fA-F]{12,14}$/.test(text)) {
      return {
        h: parseInt(text.slice(0, 4), 16),
        s: parseInt(text.slice(4, 8), 16),
        v: parseInt(text.slice(8, 12), 16),
      };
    }
  }
  return null;
    ...
    levelControl: {
      moveToLevel: async ({ level }) => {
        ...
        await bridge.sendCommands(deviceId, [
          ...
          { code: "colour_data", value: { h: Math.round(cur.h), s: Math.round((cur.s / 100) * 1000), v } },
        ]);
      },
